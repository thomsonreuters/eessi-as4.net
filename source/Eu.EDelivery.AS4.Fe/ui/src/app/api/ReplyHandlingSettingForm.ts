...
export class ReplyHandlingSettingForm {
    public static getForm(formBuilder: FormWrapper, current: ReplyHandlingSetting, path: string, runtime: ItemType[]): FormWrapper {
        return formBuilder
            .group({
                [ReplyHandlingSetting.FIELD_replyPattern]: [formBuilder.createFieldValue(current, ReplyHandlingSetting.FIELD_replyPattern, path, 0, runtime)],
                [ReplyHandlingSetting.FIELD_sendingPmode]: [formBuilder.createFieldValue(current, ReplyHandlingSetting.FIELD_sendingPmode, path, null, runtime), Validators.required],
