...
export class FormWrapper {
  ...
  private _onValueChangeSubscriptions = new Map<
    string | null,
    SubscriptionHandler
  >();
  private _onStatusChangeSubscriptions = new Map<
    string | null,
    SubscriptionHandler
  >();
  ...
  public createFieldValue(
    ...
    let result =
      current == null || byString(current, field) == null
        ? !!!itemTypes[fullPath] ||
          itemTypes[fullPath].defaultvalue === undefined ||
          itemTypes[fullPath].defaultvalue === null
          ? defaultValue
          : itemTypes[fullPath].defaultvalue
        : byString(current, field);
    return result;
  }
  ...
  public onChange<T>(
    ...
    handler: (current: T, wrapper: FormWrapper) => void
  ): FormWrapper {
    this.unsubscribeHandler(
      field,
      this._onValueChangeSubscriptions
    )._onValueChangeSubscriptions.set(
      field,
  ...
  public onStatusChange<T extends 'VALID' | 'INVALID' | 'DISABLED'>(
    field: string = this.name,
    handler: (
      current: 'VALID' | 'INVALID' | 'DISABLED',
      formWrapper: FormWrapper
    ) => void
  ): FormWrapper {
    this.unsubscribeHandler(
      field,
      this._onStatusChangeSubscriptions
    )._onStatusChangeSubscriptions.set(
      field,
      new SubscriptionHandler({
        handler
      })
  ...
  public cleanup(): void {
    for (const subscription of Array.from(
      this._onValueChangeSubscriptions.entries()
    )) {
    ...
    for (const subscription of Array.from(
      this._onStatusChangeSubscriptions.entries()
    )) {
      if (!!subscription[1].subscription) {
        subscription[1]!.subscription!.unsubscribe();
      }
  ...
  public disable(except: string[] | null = null) {
    Object.keys(this.form.controls)
      .filter(
        (key) =>
          !!!except ? true : except.findIndex((search) => search === key) === -1
      )
      .forEach((key) => {
  ...
  public reApplyHandlers(field: string | null = null): FormWrapper {
    ...
    keys.forEach((key) => {
      ...
      if (!!valueChangeHandler) {
        ...
        valueChangeHandler.subscription = this.getForm(key)
      ...
      if (!!statusChangeHandler) {
        ...
        statusChangeHandler.subscription = this.getForm(key)
          .statusChanges.distinctUntilChanged()
  ...
  private unsubscribeHandler(
    field: string,
    handlers: Map<string | null, SubscriptionHandler>
  ): FormWrapper {
