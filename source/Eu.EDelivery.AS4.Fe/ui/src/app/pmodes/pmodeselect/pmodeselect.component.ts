...
@Component({
  ...
  template: `
    <select
      class="form-control"
      data-cy="pmodes"
      (change)="selectPmode($event.target.value)"
    ...
    <select
      class="form-control"
      data-cy="pmodes"
      [ngModel]="selectedPmode"
      (ngModelChange)="selectPmode($event)"
      [attr.disabled]="!isDisabled ? null : true"
      multiselect="false"
      *ngIf="multi"
    >
...
export class PmodeSelectComponent
  ...
  public ngOnInit() {
    switch (this.mode) {
      ...
      case 'sending':
        ...
        break;
      default:
        let receivingPmodes = this.pmodeStore.changes
          .distinctUntilChanged()
          .filter((result) => !!(result && result.ReceivingNames))
          .map((result) => result.ReceivingNames);
        let sendingPmodes = this.pmodeStore.changes
          .distinctUntilChanged()
          .filter((result) => !!(result && result.SendingNames))
          .map((result) => result.SendingNames);
        this._storeSubscription = Observable.combineLatest(
          receivingPmodes,
          sendingPmodes
        )
          .filter((result) => !!result[0] && !!result[1])
          .subscribe((result) => {
            this.pmodes = result[0]!
              .concat(result[1]!)
