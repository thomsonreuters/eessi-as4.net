...
namespace Eu.EDelivery.AS4.Steps.Receive
{
    ...
    public class CreateAS4ReceiptStep : IStep
    {
        ...
        public async Task<StepResult> ExecuteAsync(MessagingContext messagingContext)
        {
            ...
            {
                Logger.Info($"{messagingContext.Logging} Receipt message has been created for received AS4 UserMessages.");
            }
        ...
        private static Receipt CreateReferencedReceipt(string ebmsMessageId, MessagingContext messagingContext)
        {
            ...
            if (useNRRFormat)
            {
                ...
                {
                    Logger.Debug(
                        $"{messagingContext.Logging} Receiving PMode {messagingContext.ReceivingPMode?.Id} " + 
            ...
            {
                Logger.Debug(
                    $"{messagingContext.Logging} Receiving PMode is configured to not use the Non-Repudiation format." + 
            ...
            {
                Logger.Debug(
                    $"{messagingContext.Logging} Because the received UserMessage has been sent via MultiHop, " + 
