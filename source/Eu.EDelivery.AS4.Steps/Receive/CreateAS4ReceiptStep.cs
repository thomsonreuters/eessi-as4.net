...
namespace Eu.EDelivery.AS4.Steps.Receive
{
    ...
    public class CreateAS4ReceiptStep : IStep
    {
        ...
        public async Task<StepResult> ExecuteAsync(MessagingContext messagingContext)
        {
            ...
            if (!receiptSigning && useNRRFormat)
            {
                Logger.Error(
                    ...
                    + $"ReceivingPMode {Config.Encode(messagingContext.ReceivingPMode.Id)} ReplyHandling.ReceiptHandling.UseNRRFormat or the ReplyHandling.ResponseSigning");
            ...
            {
                Logger.Info($"{Config.Encode(messagingContext.LogTag)} {Config.Encode(receiptMessage.MessageUnits.Count())} Receipt message(s) has been created for received AS4 UserMessages");
            }
            ...
            ReceivingProcessingMode receivingPMode)
        {
            ...
            {
                Logger.Warn(
                    $"ReceivingPMode {Config.Encode(receivingPMode.Id)} is configured to reply with Non-Repudation Receipts, "
                    + $"but incoming UserMessage {Config.Encode(userMessage.MessageId)} isn\'t signed. "
            ...
            {
                Logger.Trace(
                    $"ReceivingPMode {Config.Encode(receivingPMode?.Id)} is configured to not use the Non-Repudiation format."
                    + $"This means the original UserMessage {Config.Encode(userMessage.MessageId)} will be included in the Receipt");
            }

            if (received.IsMultiHopMessage)
            {
                Logger.Trace(
                    $"Because the received UserMessage {Config.Encode(userMessage.MessageId)} has been sent via MultiHop, the Receipt will be send with MultiHop as well");
            }

            if (useNRRFormat && received.IsSigned)
            {
                    Logger.Trace($"ReceivingPMode {Config.Encode(receivingPMode.Id)} is configured to use Non-Repudiation for Receipt Creation");
